...
/*                                                                           */
/*****************************************************************************/
void decodeAdaptativeCodeVector(bcg729DecoderChannelContextStruct *decoderChannelContext, int subFrameIndex, uint16_t adaptativeCodebookIndex, uint8_t parityFlag,
				int16_t *intPitchDelay, word16_t *excitationVector)
{
	...
	if (subFrameIndex == 0 ) { /* first subframe */
		if (parityFlag) { /* there is an error (either parity or frame erased) */
